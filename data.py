...
class SameDatasetTrainDataset(Dataset):
    ...
    def __init__(self, args: DataArguments, batch_size: int, seed: int, process_index: int = 0, num_processes: int = 1):
        ...
        context_feat_nq = datasets.Features({
            'query': datasets.Value('string'),
            'pos': datasets.Sequence(datasets.Value('string')),
            'neg': datasets.Sequence(datasets.Value('string')),
            "lang": datasets.Value("string"),   # 추가!
        ...
        for data_dir in args.train_data:
            ...
            for file in os.listdir(data_dir):
                ...
                try:
                    ...
                    try:
                        temp_dataset = datasets.load_dataset('json', data_files=file_path, split='train',
                                                             cache_dir=args.cache_path, features=context_feat_kd)
                        if not args.knowledge_distillation:
                            temp_dataset = temp_dataset.remove_columns(['pos_scores', 'neg_scores'])
                    except:
                        temp_dataset = datasets.load_dataset('json', data_files=file_path, split='train',
                                                             cache_dir=args.cache_path, features=context_feat_nq)

                if len(temp_dataset) == 0:
                    continue
                elif len(temp_dataset) < SMALL_THRESHOLD:
                    small_datasets.append(temp_dataset)
                    small_batch_size = min(small_batch_size, self.get_file_batch_size(file, batch_size,
                                                                                      train_group_size=args.train_group_size))
                else:
                    if args.max_example_num_per_dataset is not None and len(
                            temp_dataset) > args.max_example_num_per_dataset:
                        temp_dataset = temp_dataset.select(
                            random.sample(list(range(len(temp_dataset))), args.max_example_num_per_dataset))
                    train_datasets.append(temp_dataset)
                    each_data_inxs.append(np.arange(len(temp_dataset)) + cur_all_num)
                    cur_all_num += len(temp_dataset)
                    batch_size_inxs.append(
                        self.get_file_batch_size(file, batch_size, train_group_size=args.train_group_size))
    ...
    def print_batch_size(self, batch_size: int, train_group_size: int):
        length_list = ['0-500', '500-1000', '1000-2000', '2000-3000', '3000-4000', '4000-5000', '5000-6000',
                       '6000-7000', '7000-inf']
        batch_size_dict = {
    ...
    def create_batch_data(self, batch_raw_data):
        ...
        for i in range(len(batch_raw_data['query'])):
            if len(batch_raw_data['neg'][i]) == 0:
                print(f"[Warning] No negatives found for index {i}, skipping this sample.")
                continue
